...
ERA_WRITE(V12) { // Request OTA
  ...
  deviceManager.modeActive = (Mode_t)param.getInt(); // if (Mode_t)param.getInt() == 0 is Normal Mode
...
void loop() {
  ...
  if(FlagControl.Flag_Auto || FlagControl.Flag_Request_OTA || FlagControl.Flag_control_Fan || FlagControl.Flag_control_Fillter || FlagControl.Flag_control_Heater || FlagControl.Flag_control_Light || FlagControl.Flag_control_Pumb || FlagControl.Flag_control_Feeder)
  {
    ...
    for(uint8_t i = 0 ; i < 9 ; i++){
      ...
      if(i == 7) ControlCommand[i] = (deviceManager.modeActive == NORMAL_MODE) ? NORMAL_MODE_COMMAND : OTA_MODE_COMMAND;
